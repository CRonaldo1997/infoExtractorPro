...
import { promptService, PromptSet, PromptField } from '@/lib/services/prompts';
...
export default function Prompts() {
  ...
  const loadPromptSets = async (forceSelectId?: string) => {
    try {
      let data = await promptService.getPromptSets();
  ...
  const handleUpdateSetConfig = async (key: string, value: any) => {
    if (!selectedSetId) return;
    try {
      const updated = await promptService.updatePromptSet(selectedSetId, { [key]: value });
      setPromptSets(prev => prev.map(s => s.id === selectedSetId ? updated : s));
      toast.success('配置已更新');
    } catch (e: any) {
  ...
  const handleSaveFields = async () => {
    ...
    setIsSaving(true);
    try {
      await promptService.saveFields(selectedSetId, fields);
      toast.success('提示词组配置已保存成功！');
    } catch (error: any) {
  ...
  // Computed Prompt
  const systemPrompt = `从提供的文本中提取以下信息的组合。
输出要求：
1. 必须返回纯JSON格式数据。
2. 各字段的提取规则如下：
${fields.map(f => `- 字段名：${f.name} (数据类型：${f.data_type})\n  提取规则：${f.prompt}`).join('\n\n')}`;
      ...
      <main className="flex-1 overflow-y-auto p-8 space-y-8 relative">
        ...
        <section className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
          ...
          {isAdvanced && currentSet && (
            <div className="px-8 py-6 bg-slate-50 border-b border-slate-100 animate-in fade-in slide-in-from-top-2 duration-300">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 text-slate-900">
                        ...
                        <h3 className="font-bold">
                          {batchId === 'default' ? '默认提取组' : `执行批次 ${bIdx + 1}`}
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                ...
                <div className="w-full h-48 p-5 bg-slate-50 border border-slate-200 rounded-xl outline-none text-slate-500 text-sm font-mono overflow-y-auto whitespace-pre-wrap">
                  {systemPrompt}
                </div>
              </div>
