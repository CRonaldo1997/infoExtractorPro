...
import { Shield, User, Lock, EyeOff, Eye, LogIn, ChevronRight } from 'lucide-react';
...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Supabase 使用 email 登录，我们用 username@local.app 作为 email 格式
      const email = `${username.trim()}@infoex.local`;
      const { error } = await supabase.auth.signInWithPassword({ email, password });

      if (error) {
        if (error.message.includes('Invalid login credentials')) {
          setErrors({ general: '用户名或密码不正确，请重试' });
        } else {
      ...
      setErrors({ general: '登录失败，请检查网络连接' });
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="w-full space-y-8 bg-white rounded-xl shadow-sm border border-slate-200 max-w-[520px] p-10">
        <header className="flex flex-col items-center text-center space-y-4">
          ...
          <div className="space-y-1">
            ...
            <h1 className="font-black tracking-tight text-slate-900 text-4xl">欢迎回来</h1>
            <p className="text-slate-500 text-sm">请输入您的凭据以访问系统</p>
          </div>
        </header>
        ...
        <div className="flex flex-col gap-3">
          <Link href="/register" className="flex items-center justify-between w-full p-4 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors group">
            <div className="flex items-center gap-3">
              <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-primary/5 text-primary">
                <Lock className="w-5 h-5" />
              </div>
              <div className="text-left">
                <p className="text-sm font-bold text-slate-900">用户注册</p>
                <p className="text-xs text-slate-500">没有账户？点击注册</p>
              </div>
            </div>
